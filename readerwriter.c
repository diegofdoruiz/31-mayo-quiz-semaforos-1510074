...
#define MUTEX "/mysemaphore5"
#define ROOMEMPTY "/mysemaphore6"
...
void *reading(void* data) {
	...
	sem_wait(mutex); //Semaforo mutex
	contador++;
	if(contador == 1){ //
		sem_wait(roomempty); //semaforo roomempty
    }//
	printf("El lector %d estara leyendo por %li ms. Hay %d lectores en el salon\n",id,_tiempo,contador);
	__reading__(_tiempo);
	sem_post(mutex);//Semaforo mutex
